import { hopeTheme } from "vuepress-theme-hope";
import navbar from "./navbar.js";
import sidebar from "./sidebar.js";

export default hopeTheme({
  hostname: "https://box.arkitosekai.net",

  author: {
    name: "Arkito",
    url: "https://space.bilibili.com/16215189",
  },

  logo: "/assets/logo.svg",
  favicon: "/assets/favicon.ico",

  externalLinkIcon: false,

  repo: "Crystal-Cosmos/HoshinoHako-NeXT",

  repoDisplay: true,

  docsDir: "src",

  fullscreen: false,

  print: false,

  hotReload: false,

  // navbar
  navbar,

  // sidebar
  sidebar,

  footer: "「Vanitas Vanitatum Et Omnia Vanitas」",
  displayFooter: true,

  lastUpdated: true,

  editLink: true,

  contributors: true,

  darkmode: "enable",

  // page meta
  metaLocales: {
    editLink: "在 GitHub 上编辑此页",
  },

  markdown: {
    component: true,
    align: true,
    attrs: true,
    codeTabs: true,
    figure: false,
    gfm: true,
    hint: true,
    imgLazyload: true,
    imgSize: true,
    include: true,
    mark: true,
    sub: true,
    sup: true,
    tabs: true,
    tasklist: true,
    vPre: true,
  },

  plugins: {
    // You should generate and use your own comment service
    catalog: true,
    comment: false,
    copyCode: {},
    copyright: {
      global: true,
      disableCopy: true,
      disableSelection: true,
      triggerLength: 1,
    },
    //notice: [
    //  {
    //    path: "/",
    //    title: "一对一课程🎈",
    //    content: "想要一对一学习工程制作吗，我开始招收学生啦！每节两小时，教你制作自己喜欢的歌曲！",
    //    showOnce: false,
    //    confirm: true,
    //    actions: [
    //      {
    //        text: "继续了解",
    //        type: "primary",
    //        link: "https://www.bilibili.com/read/cv34722591"
    //      },
    //      {
    //        text: "关闭",
    //        type: "default",
    //      }
    //    ],
    //    fullscreen: true,
    //  },
    //],
    icon: {
      assets: [
        "//at.alicdn.com/t/c/font_4421024_lvou8ervqm.css",
      ],
    },

    components: {
      components: [
        "PDF",
        "Badge",
        "BiliBili",
        "VPCard",
      ],
    },

    watermark: {
      enabled: ({ frontmatter }) =>
        frontmatter.watermark === true ||
        (typeof frontmatter.watermark === "object" &&
          frontmatter.watermark !== null),
      watermarkOptions: {
        contentType: "text",
        movable: true,
        content: "Arkito所有，未允禁转",
        fontSize: "50px",
        mode: "blind",
        globalAlpha: 0.02,
        mutationObserve: true,
      },
    },

    // uncomment these if you want a pwa
    // pwa: {
    //   favicon: "/favicon.ico",
    //   cacheHTML: true,
    //   cachePic: true,
    //   appendBase: true,
    //   apple: {
    //     icon: "/assets/icon/apple-icon-152.png",
    //     statusBarColor: "black",
    //   },
    //   msTile: {
    //     image: "/assets/icon/ms-icon-144.png",
    //     color: "#ffffff",
    //   },
    //   manifest: {
    //     icons: [
    //       {
    //         src: "/assets/icon/chrome-mask-512.png",
    //         sizes: "512x512",
    //         purpose: "maskable",
    //         type: "image/png",
    //       },
    //       {
    //         src: "/assets/icon/chrome-mask-192.png",
    //         sizes: "192x192",
    //         purpose: "maskable",
    //         type: "image/png",
    //       },
    //       {
    //         src: "/assets/icon/chrome-512.png",
    //         sizes: "512x512",
    //         type: "image/png",
    //       },
    //       {
    //         src: "/assets/icon/chrome-192.png",
    //         sizes: "192x192",
    //         type: "image/png",
    //       },
    //     ],
    //     shortcuts: [
    //       {
    //         name: "Demo",
    //         short_name: "Demo",
    //         url: "/demo/",
    //         icons: [
    //           {
    //             src: "/assets/icon/guide-maskable.png",
    //             sizes: "192x192",
    //             purpose: "maskable",
    //             type: "image/png",
    //           },
    //         ],
    //       },
    //     ],
    //   },
    // },
  },
});
